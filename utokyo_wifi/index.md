...
一時的に特定の場所，または全学でトラブルが発生して利用に支障が出ることがありますが，ご理解ください．
...
- UTokyo WiFiを利用できるのは，UTokyo Accountを持つ東京大学の構成員（教職員・学生）です．
    - 情報セキュリティ教育を受講していない場合，構成員であっても利用が停止されることがあります．詳細は [「情報セキュリティ教育」の節](#jouhou-security) を参照してください．
...
**手順B:** アカウント情報を受け取るメールアドレスをシステムに登録する．
- UTokyo WiFiアカウントの発行にはあらかじめメールアドレスの登録が必要です．以下の手順に従って登録を行ってください．なお，**登録されたメールアドレスが有効となるのは翌日ですので，翌日以降に手順Cを実行してください．** 使用できるメールアドレスが既に登録されている場合は，そのままにして手順Cに進んで構いません．
<details><summary>教職員の場合</summary>
    「東大ポータル」→「人事労務・研修」→「個人情報・職員証発行申請」→「ログイン」→「人事情報システムログイン」→「パーソナルメニュー」→「学内メールアドレス」を設定してください．</details>
<details><summary>学生の場合</summary>
    学務システムUTAS→「学生情報」→「現住所等変更入力」→「E-Mail1」を設定してください．（E-Mail2のアドレスはUTokyo WiFiのメール送信先としては使えません）
</details>

**手順C:** UTokyo WiFiアカウントの発行を申請する．
- 以下の「UTokyo WiFiアカウントメニュー」からUTokyo Accountでログインし，「新規申請」ボタンを押して，アカウントを新規に取得してください．なお既にアカウントを持っている場合，または2台目以降の場合は発行手続きは必要ありません．
- UTokyo WiFiのアカウントの情報（ユーザ名, パスワード）が記載されたメールが前もって登録されたメールアドレスに届きます．有効期限は半年です．毎年4月末，10月末に失効します．
...
<ul>
    ...
    <li> 以下のような画面が出てアクセスできない場合
        <img src="img/account_menu_login_denied.png">
        <ul>
            <li> 情報セキュリティ教育を受講していないため，アカウントが停止されています．詳細は <a href="#jouhou-security">「情報セキュリティ教育」の節</a> を参照してください．</li>
        </ul>
    </li>
</ul>
</details>
...
**手順D:** UTokyo WiFiが接続可能なネットワークとして表示されている場所で接続をする．
以下の記述に従って，教室などUTokyo WiFi（SSIDがUTokyo-WiFiとなっているワイヤレスネットワーク）が見えるところで接続を行います．
接続しようとするとアカウント情報を聞かれるので，メールに書かれたアカウント情報を入力します．
...
<ul>
    ...
    <li> また，eduroamと同様にして設定ができます．<a href="https://www.eduroam.jp/for_users/">eduroamのページ</a> の利用の手引きにOSごとの設定方法が書いてありますので，"eduroam"を"UTokyo-WiFi"に読み替えることで設定が可能です．
    </li>
</ul>
</details>
...
<ul>
    ...
    <li> また，利用する機器が最新の状態ではないために起こる場合もあります：
        <ul>
            <li> 古いMacOS，Windows 7以前，アップデートのない機器（スマホ等）は接続までに，最新のバージョンにしてください．</li>
        </ul>
    </li>
</ul>
</details>
...
- **UTokyo WiFiのアカウントは半年に一度，4月末と10月末に失効します**．毎年4月1日，10月1日以降に，半年後まで有効なUTokyo WiFiアカウントが再発行できるようになります．時期が来たら「UTokyo WiFiアカウントメニュー」から「新規申請」ボタンを押して再発行してください．アカウントを再発行したら，新しく発行されたユーザ名，パスワードを端末に再度設定してください．
    - ただし，その年の情報セキュリティ教育を受講していない場合，10月1日から「UTokyo WiFiアカウントメニュー」にログインできなくなり，したがって再発行ができません．該当者は [「情報セキュリティ教育」の節](#jouhou-security) を参照し，必ず受講するようにしてください．
