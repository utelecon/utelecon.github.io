...
- ワークスペースのURL
    ...
    - 英字（小文字）・数字・ハイフンのみ使用可能です．少なくとも英字を必ず1文字入れてください．ハイフンで始まったり終わったりすることはできません．文字数は21文字までです．
...
申請の承認後，ワークスペースが作成され，申請者（申請フォームを送信した方）に対して招待が送られます．申請者は，「[UTokyo Slackに特定のワークスペースから招待を受けて参加する](/slack/invited)」のページの手順に従って，作成されたワークスペースに参加してください．
...
ワークスペースを一覧画面に表示するかどうかや，一覧画面からの参加時に個別の許可が必要かどうかは，ワークスペースごとに設定できます．
...
~~2022年8月26日より，ワークスペース作成申請の受付を開始します．~~ <strong>8月26日18時現在，申請システムの修正のため一時的に申請受付を中止しています．再開日時は未定です．</strong>

作成が完了すると，「[UTokyo Slackワークスペースディレクトリ](https://utokyo.enterprise.slack.com)」に表示されます（申請者にのみ表示されます）．申請に問題があった場合にはECCSクラウドメールにメールを送付しますのでご確認ください．フォームへの記載は，代表責任者ご本人にお願いいたします（代理申請はできません）．なお，フォーム作成は自動処理となっておりますが開始当初は確認しながら動作させますので時間がかかる可能性があることはご了承ください．
