...
3. When requested to update your password, enter the initial password from the notification into the “Current password” field, and enter your new password in the field marked “New password” and "Confirm password".
    ...
    <Fragment slot="else">However, the ECCS printing service (printer operation panel), the financial accounting system and budget execution management system limit a password to 32 characters, so we recommend the password be set to 32 or less characters when using these systems. ※If you set your password using the initial password, it may take some time (up to approximately 30 minutes) before the system becomes available. This is because it can take up to 30 minutes for the MFA settings to be reflected in the system. Therefore, for those who are setting up MFA next, the delay caused by password setup should have minimal impact. Please proceed with the MFA setup as is, and wait for a while until it takes effect (MFA activation is mandatory).</Fragment></If>
