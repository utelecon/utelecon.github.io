...
※If you set your password using the initial password, it may take some time (up to approximately 30 minutes) before the system becomes available. This is because it can take up to 30 minutes for the MFA settings to be reflected in the system. Therefore, for those who are setting up MFA next, the delay caused by password setup should have minimal impact. Please proceed with the MFA setup as is, and wait for a while until it takes effect (MFA activation is mandatory).
