import If from "@components/utils/If.astro";
import Support from "@components/utils/Support.astro";
import Switch from "@components/utils/Switch.astro";
import UtacPasswordNotice from "./20230321UtacPasswordNotice.mdx";

{/**
  * `If`, `Support`, `Switch`の利用例として紹介されています．
  * これらのコンポーネントを利用しなくなる場合は，`src/components/README.md`を編集して，
  * 別の利用例を紹介するようにしてください．
  *
  * 利用ファイル：
  * - `./index.mdx`
  * - `src/pages/faculty_members/index.mdx`
  *
  * 利用ページ：
  * - https://utelecon.adm.u-tokyo.ac.jp/oc/#important
  * - https://utelecon.adm.u-tokyo.ac.jp/oc/#utokyo_account
  * - https://utelecon.adm.u-tokyo.ac.jp/faculty_members/#初期パスワードの変更
  * - https://utelecon.adm.u-tokyo.ac.jp/utokyo_account/#procedures
  */}

{/**
  * @typedef {object} Props
  * @property {string} variant ページの種類 パスワード通知書を受け取る手順での説明が異なるため必要
  * @property {boolean} [support] サポート窓口の案内を表示するか否か 窓口への案内を表示したくないページもあるため必要
  * @property {boolean} [movie] 手順動画へのリンクを表示するか否か 手順動画へのリンクを表示したくないページもあるため必要
  * @property {boolean} [blockLink] UTokyo Account利用者メニューへのリンクをブロックにするか(true)，説明テキストに含めるか(false) ページによって目立つべきか小さく収まるべきかが異なるため必要
  */}

export const students = <Fragment>授業開始に十分間に合うように配布されるはずですが，不明な場合は所属の学部・研究科等に問い合わせてみてください（※学部1年生には3月13日から<a href="https://utas-ew.adm.u-tokyo.ac.jp/">合格発表の際に情報登録を行ったWebシステム</a>で配布されます）．</Fragment>
export const faculty_members = <Fragment>授業に必要な教員には発行されているはずですが，もらっていない場合は所属部局の事務に問い合わせてください．</Fragment>

<If cond={props.movie}>
  - [手順動画](/oc/movies#utokyo_account)
</If>

1. 所属の学部・研究科等から，初期パスワードが記載された「パスワード通知書」を受け取ってください．
   <Switch variant={props.variant}>
     <Fragment slot="oc">{students}</Fragment>
     <Fragment slot="faculty_members">{faculty_members}</Fragment>
     <Fragment slot="individual">
      - **学生**：{students}
      - **教職員**：{faculty_members}
     </Fragment>
   </Switch>
2. <If cond={props.blockLink}>
     以下の「UTokyo Account利用者メニュー」に，「パスワード通知書」に記載されたユーザ名とパスワードでログインしてください．
     <b class="box center">
       [UTokyo Account利用者メニュー](https://utacm.adm.u-tokyo.ac.jp/webmtn/LoginServlet)
     </b>
     <Fragment slot="else">
       [UTokyo Account利用者メニュー](https://utacm.adm.u-tokyo.ac.jp/webmtn/LoginServlet)に，「パスワード通知書」に記載されたユーザ名とパスワードでログインしてください．
     </Fragment>
   </If>
3. 左上の「パスワード変更」を押して，パスワードを変更してください（初期パスワードのままでは，各種サービスを利用することができません）．新しいパスワードを決める際には，パスワードの強度を示す表示が「強い」または「とても強い」になるようにしてください．
   <UtacPasswordNotice />

<Support lang="ja" show={props.support} />
