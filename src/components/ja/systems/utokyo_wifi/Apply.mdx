...
1. 表示された画面で，「通知用メールアドレス」欄に記載されているメールアドレスが正しいか確認してください．その際、以下の点に留意してください．
   - UTokyo Wi-Fiアカウントの情報（ユーザーID・パスワード）は，このあとに行う申請送信**直後**のアカウントメニューの画面と，申請後に届く通知メールの両方で閲覧できます．
   - 申請送信直後のアカウントメニュー画面を閉じてしまうと，通知メール以外からはアカウントのパスワードを閲覧できなくなります．
   - 通知メールは[ECCSクラウドメール](/google/)のメールアドレスと，「通知用メールアドレス」欄に記載されているメールアドレスの両方に届きます．
     <details>
