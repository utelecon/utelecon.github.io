...
- UTokyo Wi-Fiを利用できるのは，[UTokyo Account](/utokyo_account/)を持つ東京大学の構成員（教職員・学生）です．
    - UTokyo Accountの多要素認証（MFA）を設定していない場合，アカウントを発行することができません．手順については[こちら](https://utelecon.adm.u-tokyo.ac.jp/utokyo_account/mfa/initial/)を参照してください．
...
### UTokyo Accountの多要素認証（MFA）
{:#mfa}

UTokyo Wi-Fiの利用にあたっては，UTokyo Accountの多要素認証（MFA）設定を必須としています．東京大学では，情報システムのセキュリティ向上を目的として，UTokyo Accountに多要素認証を導入しています．
...
- 多要素認証を設定していない場合はUTokyo Wi-Fiの利用資格が停止されます．具体的には，UTokyo Wi-Fiアカウントメニューへのサインインやアカウントの新規発行ができなくなります．
- UTokyo Wi-Fiを利用する前には**必ず**多要素認証の設定を行ってください．具体的な手順については[UTokyo Account多要素認証の初期設定手順](https://utelecon.adm.u-tokyo.ac.jp/utokyo_account/mfa/initial/)を確認してください．
...
- アカウントを発行し直したら，新しいアカウントのユーザーID・パスワードを利用している機器に入力し，接続してください．
    - 以前のUTokyo Wi-Fiアカウントの情報を機器が記憶してしまっており，情報を変更できない（新たなユーザーID・パスワードを入力するための画面が出ない）場合，一度Wi-Fiのアカウント情報を削除してください．具体的な削除方法はOSや機器の種類によって異なりますので，[端末の種類ごとの説明ページ](#procedure-by-device-type)の手順3を参照してください．これらのページに記載がない場合は、それぞれの機器のマニュアルを参照してください．
