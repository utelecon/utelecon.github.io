...
**手順C:**Cisco Secure Clientのインストールの途中で，以下のような「Action Required AnyConnect VPN Service Not Enabled」というポップアップが表示されます．ポップアップの指示に従って，「システム設定」→「一般」→「ログイン項目と機能拡張」にすすみ，「アプリのバックグラウンドでのアクティビティ」のCisco Secure Client - AnyConnect VPN Serviceを有効にしてください．この際，「"Cisco Secure Client -Socket Filter"が新しいネットワーク機能拡張を使おうとしています」という画面が出る場合があります．表示される手順にしたがって，「システム設定」→「プライバシーとセキュリティ」のペインに表示されている"Cisco Secure Client - Socket Filter"に関する許可を行ってください．

![](img/mac04_0-install-2.png){:.medium.center.border}
![](img/mac04_1-install-2.png){:.medium.center.border}

**手順D**
以下のような「Cisco Secure Clientセキュアモビリティクライアント通知」というポップアップが表示されたら，表示される手順にしたがって，「システム設定」→「一般」→「ログイン項目と機能拡張」にすすみ，「機能拡張」の「カテゴリ別」を選択し「ネットワーク機能拡張」の右にあるiボタンをクリックしてCisco Secure Clientを有効にしてください．
