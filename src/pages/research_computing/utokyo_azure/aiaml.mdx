...
- **名前**：リソースのポータルでの名称を設定します．ここでは`test_workspace`としています．
...
一度ワークススペースを作成すれば他のユーザと共有が可能となり，共有ユーザーは画面の Users の下に他のユーザのフォルダが表示される．ただし Notebook を利用するには各自が実行用の仮想マシンをワークスペース内に作らなければなりません．
