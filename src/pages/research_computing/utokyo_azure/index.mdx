...
<div class="box">
**重要**：割り当てられたサブスクリプションには、UTokyo Azure 独自の機能として無料保証枠の90%を使い切った時点でサブスクリプションを自動停止するという機能があり、初期設定では**有効**になっております。無料保証枠内で利用を確実に止めたい利用者への機能となりますが、停止されると即時にサブスクリプションが取り消され、復旧ができなくなりリソースへのアクセスも不可になります。**データ等のリソースを取り消されたくない場合は、サブスクリプションを作成後忘れずに[**UTokyo Azure新規利用申請ページの管理ページ**](http://azure.itc.u-tokyo.ac.jp/list)から当該サブスクリプションの自動停止機能を無効に設定変更してください。**
</div>
