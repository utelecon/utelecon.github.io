...
1. Azureサービスの一覧の中から`Azure AI Foundry`を選択します．
...
2. Azure AI Foundry のメインページが表示されたら，画面左にあるリストから`Use with Foundry` → `Foundry`をクリックします．
...
ここでは，新規利用の想定で初期設定のまま進めます．
- 既に Azure 上に展開されている既存データやリソースを利用する場合は，既存データにアクセスするための認証が必要な場合は`Key Vault`や，データが置いてあるAzureストレージへアクセスするために[`Storage Account`](/research_computing/utokyo_azure/storage/)情報が必要な場合はその設定を，必要に応じて行ってください．
...
1. ポータルによる検証が成功したら，画面左下の`作成`ボタンをクリックし，しばらくしてデプロイが正常に完了しましたら，`リソースに移動`をクリックしてください．
2. Foundry のトップページに遷移するので，`Go to Foundry Portal`をクリックしてください．
