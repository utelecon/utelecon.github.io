...
<HelpItem lang="ja" type="details">
  ...
  <Fragment slot="solution"><p>同一リージョン内で，ネットワーク，電源，冷却装置等が独立しているデータセンター群で，イメージとしては隣接している複数のビルそれぞれに構築されているデータセンターの集まりです．</p><p>通常3ゾーン以上で構成されていて，可用性ゾーンを複数指定するとそのゾーン間でプライマリバックアップやHAを構成することが可能になります．</p><p>本利用例の通りに設定した場合はゾーンの利用は行われず，いずれかのゾーン内に単独で構成されます．</p></Fragment>
</HelpItem>
- **イメージ**：利用したいＯＳを選択してください．ここでは ubuntu OS を選択しています．
<HelpItem lang="ja" type="details">
  <Fragment slot="problem">WindowsOS を選択した場合</Fragment>
  <Fragment slot="solution">ページ最下部に**マルチテナントをホストする権利を持つ有効な Windows 10/11 ライセンスを所有しています**という項目があるのでチェックを入れてください．UTokyo Azure では [UTokyo Microsoft License](https://utelecon.adm.u-tokyo.ac.jp/microsoft/) で認証されたOSを利用できます．また，WindowsOS は必要最小スペックが高めですので、次項目のVMサイズを選択する際は，10/11 での最小スペックにご注意ください．特に Windows11 の場合は [Microsoft Learn：Azure 仮想マシンでの Windows 11 のサポート](https://learn.microsoft.com/ja-jp/troubleshoot/azure/virtual-machines/windows/windows-11-support-azure-virtual-machines)を参照ください．</Fragment>
</HelpItem>
