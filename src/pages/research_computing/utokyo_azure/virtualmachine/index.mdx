...
<HelpItem lang="ja" type="details">
  ...
  <Fragment slot="solution">同一リージョン内で，ネットワーク，電源，冷却装置等が独立しているデータセンター群で，イメージとしては隣接している複数のビルそれぞれに構築されているデータセンターの集まりです．通常3ゾーン以上で構成されていて，可用性ゾーンを複数指定するとそのゾーン間でプライマリバックアップやHAを構成することが可能になります．本利用例の通りに設定した場合はゾーンの利用は行われず，いずれかのゾーン内に単独で構成されます．</Fragment>
</HelpItem>
