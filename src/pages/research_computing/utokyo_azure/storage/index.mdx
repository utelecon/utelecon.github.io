...
- 適切なサブスクリプション，リソースグループを選択し，[ストレージアカウント名](https://learn.microsoft.com/ja-jp/azure/storage/common/storage-account-overview#storage-account-name)を入力します．
- ストレージアカウント名は，以下の[命名規則](https://learn.microsoft.com/ja-jp/azure/storage/common/storage-account-overview#storage-account-name)に基づき自由な文字列で設定可能です．
  - 3 文字以上 24 文字以下である必要があり，数字と小文字のみ使用可能．
  - Azure 内で一意である必要があり，他のユーザー含めてほかのストレージアカウントと重複する命名は不可．
