...
## はじめに
このページでは，UTokyo Account本人確認サービスを既に利用している方が，「事前連係」を解除する手続きについて説明します．
...
{:#app-setup}
まず，デジタル認証アプリのセットアップを行います．事前連係の手順1と同様のため，既にセットアップを済ませたスマートフォンで連係解除を行う場合は，省略し，[手順2](#auth)に進んでください．
...
1. 「上記の紐付けを解除する」ボタンを押して，連係を解除してください．
    ![](service_step2_unlink.png){:.border}{:.medium}

    <details>
    <summary>「上記の紐付けを解除する」ボタンが出ない場合</summary>
    そもそも紐付けが行われていない状態です．[手順4](#statuscheck)に従って, UTokyo Account利用者情報確認サイト上で連係状況を確認することをおすすめします．
    </details>

    {/* ここから共通 */}
1. 「上記の紐付けを解除する」ボタンが出なくなれば，連係を解除できています．
...
1. 「UTokyo Account本人確認サービスを利用していません」と表示されていれば，連係解除が完了しています．
