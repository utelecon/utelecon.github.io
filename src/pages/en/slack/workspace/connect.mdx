...
This section explains how to configure permissions for members from external workspaces participating in shared channels. Specifically, you can choose whether members from external workspaces have "Permission to post" or "Permission to post, invite, and more" in the shared channel. For more details, please refer to [Slack Connect: Manage channel invitation settings and permissions](https://slack.com/help/articles/1500012572621-Slack-Connect--Manage-channel-invitation-settings-and-permissions).

By default, shared channels configured using the above steps are set to allow "Permission to post, invite, and more" for members from external workspaces, providing advanced management capabilities. If necessary, consider changing this setting.
...
1. In the dialog that appears, select the "Channel Details" tab. Then, in the "Organization" section at the bottom, click the menu button (three-dot icon) next to the name of the external workspace.
1. From the displayed menu, select "Manage Permissions".
1. Choose either "Permission to post" or "Permission to post, invite, and more.", and then click "Next".
