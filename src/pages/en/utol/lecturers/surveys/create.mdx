...
For information on how to view survey results, please refer to "[Viewing Aggregated Results](../result/)".  
...
- **Anonymous respondents cannot revise their answers.**
...
Next, click "Create New" in the "Question Settings" field at the bottom of the "Surveys Registration" screen. The "New Registration" screen for questions will be displayed.
...
1. First, decide on the format of the questions you wish to create. Select the appropriate format from the following seven options according to the content you want to ask.
  - **Single selection (radio buttons)**: Select one option from a list.
  - **Multiple selection (check boxes): Select multiple options from a list.
  ...
  - **List selection (pull-down)**: Similar to single selection, this format allows you to select one option from multiple options, but a pull-down menu is displayed instead of radio buttons.
  - **Single-selection table (radio buttons)**: This format displays radio buttons in a table, allowing you to select one option per row.
  - **Multiple-selection table (checkboxes)**: This format displays checkboxes in a table, allowing you to select multiple options.

2. Enter the question information (the screenshot below shows a single-selection format).
  ![](./create-question-info.png)
  - **Question Format**: Select the question format you decided on in step 1.
  - **Required Answer**: If you select this option, you must answer the question before the form can be submitted.
  - **Title**: The title of the question.
  - **Contents**: The explanation that appears above the question. Markup can be used.
  - **Number of Answers/Answered items**: Settings for the question options, etc.
    - The input content varies depending on the question format as follows.
      - **Single selection/Multiple selection/List selection**: Enter the number of the answer items (options) and the content of the answer items.
      - **Text/Free text**: No additional input fields are available.
      - **Single-selection table/Multiple selection table**: Enter the number and content of question items in the vertical columns, and the number and content of answer items in the horizontal rows.
    - Note
  ...
  - **Comment field**: When enabled, a free-text input field (text area) will appear. The content entered here can be reviewed by the instructors or TAs as individual responses. Note that this is only available for selection formats and cannot be used with text or free-text formats.
  ...
  - **Display Table/Graph**: Set the format for displaying the survey results. This cannot be used for text or free text formats.
    ...
    - **Number of responses (table)**: The number of respondents and the percentage will be displayed for each answer option.
    - **Proportion (circle graph)**: In addition to the "Number of answers" table, a circle graph showing the percentages will be displayed.
...
4. If necessary, set branching survey questions (the next question to be displayed).
  - In case of single-selection and list selection questions, you can specify a different question to jump to for each answer choice.  This is useful when you wish to change the next question to be displayed depending on the respondent’s answer.
...
If you find any problems in the Answer Simulation, please refer to "Editing created surveys" below to correct the survey.
...
- **After** the start of the response period and before it ends: The following 7 items only
  ...
  - Enabling or disabling disclosure of survey results
...
2. Click the template title you wish to edit or delete.
  - **To edit a template: Edit the displayed settings**: Only the title and questions can be edited. Other items displayed on the "Survey Registration" and "Editing" screens cannot be edited because they are not included in the template.
...
3. The "Surveys Template Registration/Editing" screen will appear. Make any necessary changes, such as changing the title to something easier to understand, and then click "Confirm".
