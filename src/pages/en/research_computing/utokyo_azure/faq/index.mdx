...
<HelpItem lang="en" type="details">
  ...
  <Fragment slot="solution"><p>"Weight" setting reflects unique, valid accounts actively registered to a subscription or resource. Please set the role to "Active." </p><p>If the same account is registered to multiple scopes within the same subscription, duplicates are eliminated across the subscription when calculating the weight. If the same account is registered to different subscriptions, it is treated as valid within each respective subscription.</p><p>The recalculation of "weight" settings takes place at a specific time on the first day of each month. Changes are not reflected immediately after configuring a role, so please wait until the following month.</p></Fragment>
</HelpItem>
