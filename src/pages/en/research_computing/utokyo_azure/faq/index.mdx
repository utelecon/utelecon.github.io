...
<HelpItem lang="en" type="details">
  ...
  <Fragment slot="solution"><p>Weight settings are based on individual accounts that are actively registered and valid within a subscription. Please set the role to "Active."</p><p> If the same account is registered to multiple roles within the same subscription, duplicates are excluded from the weight calculation. If the same account is registered across different subscriptions, it is treated as valid within each respective subscription.</p><p>The recalculation of "weight" settings takes place at a specific time on the first day of each month. Changes are not reflected immediately after configuring a role, so please wait until the following month.</p></Fragment>
</HelpItem>
