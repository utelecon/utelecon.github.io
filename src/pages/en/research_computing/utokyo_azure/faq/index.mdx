...
<HelpItem lang="en" type="details">
  ...
  <Fragment slot="solution"><p>Can be used for research and education purposes. </p><p>It can also be used for administrative tasks purposes as long as it is for operational and functional testing. If you wish to move to full-scale use after testing, please sign a separate contract for Microsoft Azure instead of this service.</p></Fragment>
</HelpItem>
...
<HelpItem lang="en" type="details">
  ...
  <Fragment slot="solution"><p>This is the free portion of the usage fee for a given month, calculated based on all users at the end of the month.</p><p>Each user must pay any usage fees that exceed the free portion from their own research funds, etc. In practice, the excess and surplus amounts for each month are tallied for each half-year at the end of March and September, and the total excess amount for each half-year is calculated. This total excess amount will be invoiced by the Information Infrastructure Center. For further details, please refer to the [UTokyo Azure information session materials.](/events/2025-04-18/slides/2_possibility_of_charges.pdf) </p><p>The Free Tier is calculated at the end of each month to be fair, taking into account the university's overall usage fee and number of subscriptions for that month. </p><p>The usage fee for each month can be confirmed on the management page of the [UTokyo Azure : New User Application Page (limited to access from the campus network)](http://azure.itc.u-tokyo.ac.jp/).</p></Fragment>
</HelpItem>
...
<HelpItem lang="en" type="details">
  <Fragment slot="problem">The "weight" setting is not being reflected, or the figures do not match my own calculations.</Fragment>
  <Fragment slot="solution"><p>The "weight" setting reflects unique accounts that are validly registered to a subscription or resource. Invalid roles—such as those configured with "Eligible" status—are not included in the calculation. Additionally, if the same account is registered to multiple resources, it is deduplicated across the subscription. However, if the same account is registered to different subscriptions, it is counted as valid for each one.</p><p>The recalculation of "weight" settings takes place at a specific time on the first day of each month. Changes are not reflected immediately after configuring a role, so please wait until the following month.</p></Fragment>
</HelpItem>

<HelpItem lang="en" type="details">
  <Fragment slot="problem">I would like to know how much Guaranteed Free Tier use is left and how much I can use for free.</Fragment>
  <Fragment slot="solution"><p>Owner only: The [UTokyo Azure New User Application Management Page](http://azure.itc.u-tokyo.ac.jp/list) will show you how much your subscription has used so far. It also shows a graph of how much of the free credits have been used by all users overall. This information is updated daily.</p><p>In addition, a unique feature of UTokyo Azure is that when the amount of gift credits consumed reaches 50%, 75%, 90%, or 100% of the Guaranteed Free Tier for that month, a notification email is sent to the Owner's registered email address.</p></Fragment>
</HelpItem>

<HelpItem lang="en" type="details">
  <Fragment slot="problem">I want to know how much gift credits a particular resource costs</Fragment>
  <Fragment slot="solution">Owner only: Detailed information on a resource-by-resource basis is displayed in table format at the end of the [UTokyo Azure New User Application Management Page](http://azure.itc.u-tokyo.ac.jp/list), so please refer to the "Cost" section. The amount displayed is the amount consumed on that day, so if you want to know the total amount, add up the costs for the same resource for the period you want to know about.</Fragment>
</HelpItem>

<HelpItem lang="en" type="details">
<Fragment slot="problem">I share my subscription with multiple people and I want to know how many gift credits each person has used.</Fragment>
  <Fragment slot="solution">The smallest unit at which you can check gift credit consumption is the subscription, and it cannot be checked on a user account basis. However, if you are the only one using a specific resource, it is possible to check on a resource basis. For details, please refer to `FAQ: I want to know how much gift credits a particular resource costs`.</Fragment>
</HelpItem>
...
<HelpItem lang="en" type="details">
  ...
  <Fragment slot="solution"><p>Yes. You can use the [UTokyo Microsoft License](/en/microsoft/) with UTokyo Azure. When configuring a virtual machine, check **"I confirm I have an eligible Windows 10/11 license with multi-tenant hosting rights."**</p><p>Official support for Windows 10 ended in October 2025. Unless there are special circumstances, please use Windows 11.</p></Fragment>
</HelpItem>
...
<HelpItem lang="en" type="details">
  <Fragment slot="problem">Why do errors sometimes occur when proceeding with the steps to request a VM size quota?</Fragment>
    <Fragment slot="solution">A VM size quota request can be made through various steps, and we have received inquiries from users about problems such as some steps not proceeding as expected or links leading to errors. If you encounter such a situation, please contact the UTokyo Azure service staff and tell them which step caused the problem. Then, proceed with the quota request using the alternative steps below.<ul><li>[(Japanese) VMサイズのクォータ要求でエラー表示が出る場合の代替手順](https://univtokyo.sharepoint.com/:b:/t/Teams.azure.adm/EZQGpSZd0rBGg02rzaYJabgB26SajE7VYwNiJlLBGemJOg?e=QZAldo)</li><li>Authentication with a UTokyo Account is required to access this document</li></ul></Fragment>
</HelpItem>

<HelpItem lang="en" type="details">
  <Fragment slot="problem">Can I use an external tenant (External Entra ID)?</Fragment>
    <Fragment slot="solution">Do not create an external tenant linked to UTokyo Azure. Even if you are able to create one, you will not be given the authority to create subscriptions in that external tenant, so it will be pointless. Also, even if you manage to create a subscription somehow, that subscription will not be linked to UTokyo Azure, and the full usage fee will be charged to the user.</Fragment>
</HelpItem>
