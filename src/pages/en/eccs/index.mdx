...
{:#environments}
As part of ECCS2021, more than 1,000 ECCS have been installed across the Hongo, Komaba, and Kashiwa Campuses. For more details on facilities where ECCS terminals are available, please refer to the "[Available facilities to use ECCS terminals and ECCS](./overview/facilities/)" page (currently available only in Japanese). Each facility where ECCS terminals are installed also has UTokyo Wi-Fi access points, allowing users to connect to the Internet using their own devices.
...
To use ECCS, you must first obtain the Right to use the ECCS. Once granted, you can access ECCS using your UTokyo Account username (a 10-digit number common ID) and password. For more details on obtaining Right to use the ECCS, please refer to "[Apply for Right to use the ECCS](./overview/right-application/)".

In ECCS2021, Chromebox terminals are installed in addition to iMac and mac mini ECCS terminals, allowing users to access the ChromeOS environment. The same environment is available on any terminal in any terminal room. Unlike iMac and mac mini terminals, Chromebox terminals require users to log in with their [ECCS Cloud Email](/google/) account. For more details on how to use Chromebox terminals, please refer to "[Chromebox Terminals](./chromeos)"(currently available only in Japanese). 

### Notices When Using
{:#precautions}

Please refer to the "[Notices When Using](./overview/precautions/)" page (currently available only in Japanese).
...
For more details, please refer to the "[Printing](./features/printing/)" page (currently available only in Japanese).
...
{:#ras}
ECCS provides a system that allows users to access a GUI environment similar to the macOS environment of the ECCS terminals using an exclusive client. For more details, please refer to the "[Remote Access Environment" page](./features/ras/)" (currently available only in Japanese).
...
For more details , please refer to the "[Use of SSH Server](./features/ssh/)" page (currently available only in Japanese).

#### WebDAV Environment
ECCS provides a WebDAV environment that allows users to access files on ECCS terminals from outside the system and transfer files between the Mac environment and the Windows environment, which have separate home directories.  For more details, please refer to the "[Use of WebDAV Environment](./features/webdav/)" page (currently available only in Japanese).

#### Guest Account
{:#guests}
To use ECCS, you must first obtain the Right to use the ECCS. However, a special account is available for users who need temporary access to the macOS environment, such as for obtaining Right to use the ECCS.  For more details, please refer to the "[Guest Account](./overview/guest-account/)" page (currently available only in Japanese).  
