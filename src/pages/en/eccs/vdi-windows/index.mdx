...
- **OneDrive** (some folders will automatically be synchronized with the cloud)
  - OneDrive provided under the UTokyo Microsoft License is automatically available on the virtual environment, which is linked to the UTokyo Account used to sign in to it.
  - The **Desktop, Document** and **Picture folders** are automatically synchronized with OneDrive.
...
- **Downloading via File Transfer**
  ...
  - Please note that you need to download files each time you make changes in the remote
    environment in order to save them.

In addition, when using Microsoft Office365 apps (such as Word or PowerPoint), you can save files directly to OneDrive from within the app.

Please note that OneDrive and Google Drive have storage limits. Therefore, please be careful not to place unnecessarily large files in folders that are synchronized with the cloud.

#### Unable to Install New Apps

You cannot install new applications on the Remote Windows Environment (including from the Microsoft Store).
In addition, actions that require administrative privileges (such as creating a WSL environment) are not permitted.
...
For access to e-journals, please consider using EzProxy. For other services, please consider connecting from your local devices using the UTokyo VPN instead of the Remote Windows Environment.
...
- Memory: 8 GB
- Storage: 256 GB
  - Please note that, as described in "Differences From a Typical Windows Device", data may be deleted after a certain period of time has passed since the connection was disconnected or since the last recorded user activity.
...
- MATLAB: Sign in using your own MathWorks account. At the University of Tokyo, MATLAB is available at no additional cost under the UTokyo MATLAB license. For details on how to start using it, please refer to the UTokyo MATLAB License webpage.
- Autodesk Fusion: Sign in using your own Autodesk account. As of March 2026, it is available free of charge upon registration for Autodesk Student Access.
- JMP Pro: Sign in using your own My JMP account. As of March 2026, it is available free of charge upon registration for JMP Student Edition.
- Stata/SE:A license is provided by the University.
- EndNOTE online: A licence is provided by the University.
...
- Free Slots (3 slots x 6 groups \= 18 slots in total): Available without reservation.
  ...
  - In the screen of the Windows App, they are displayed as "ECCS-FREEn" (where n is a number from 1 to 6). These options may not appear or connections may fail if the slots are full. For more details, please refer to the "How to Use" section.
- Reserved slots (182 slots): Reservation in Advance required
  - Reservation methods differ depending on what the virtual environment will be used for (e.g. for courses vs. for personal use). Please refer to the “Reservations” page for details.
