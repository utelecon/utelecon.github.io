...
* 開催者・参加者共通
  ...
  * ZOOM ソフトウェアを接続用デバイスへインストールする
    * <a href="https://zoom.us/download" target="_blank">ダウンロードページ</a>へ移動して，ミーティング用Zoomクライアントをダウンロードして ZOOM をインストールしてください

* 開催者
  * アカウントを作成する（具体的な方法は[こちら](zoom/create_account)）
  * 会議室を作成して，周知する（具体的な方法は[こちら](zoom/create_room)）
  * 会議室へ入る（具体的な方法は[こちら](zoom/join)）

* 参加者
  * 会議室の情報（URLなど）をメールなどを用いて開催者から得る
  * 会議室へ入る（具体的な方法は[こちら](zoom/join)）
