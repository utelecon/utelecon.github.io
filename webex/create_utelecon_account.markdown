...
1. サインインしたら，右上にある“undefined”と書かれた部分を選び，自分のプロファイルを選択します．開いたページの中央下側に表示される“プロファイルを編集する”を選択し，表示名（オンライン会議で表示されます）や姓名（システム管理で用いられます）を設定します．（注意：メールアドレスを変更するとアカウントが使用できなくなります．）
1. 以上で，初期設定は終了です．
