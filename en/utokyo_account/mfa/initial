---
title: Initial Setup Procedure of Multi-Factor Authentication for UTokyo Accounts
css: [mfa]
---
...
This page provides information about initial procedures to enable MFA. The outline is as follows.

- In Step 1 and Step 2, you register the “verification method” for your UTokyo Account. There are following verification methods, so please register two or more methods so that you have an alternative in case one of the verification methods is unavailable (the type of the alternative verification method can be the same as the primary method).
    ...
    - **Other Authenticator Apps**If you already use other authenticator apps, such as “Google Authenticator”, you may also use it for your UTokyo Account authentication. If you have more than one phone, you can register the same authenticator apps on these phones, making them two or more verification methods.
    - **Phone Number**: You can add your phone number and receive a SMS message or call (voice guidance) to verify your identity.
    - (For faculty members) **Hardware Token**: If you are a faculty member and none of the verification method above is available, we offer a device called “Hardware Token” for verification. The initialization process is different from other methods, so please follow [the guidance in UTokyo Portal (faculty members only)](fido-security_key) to get started.
...
**Complete all the tasks from Step1 to Step 4.** After you complete the whole procedure, **you will be asked to verify your identity with the registered SMS, apps, etc., whenever you sign in to your UTokyo Account.**Be well aware that your UTokyo Account will be inaccessible in the case you lost access to every verification method (smartphone apps, phone number, etc.).
...
First, set up the verification method for MFA. Here, you will add the first verification method (the second and subsequent methods will be added in Step 2). The first verification method will become the default (automatically selected) method, so it is best to register the method you will use most often.
...
<li>The next procedure differs by the device that you are working on. 
<ul>
<li>
...
<summary>Setup Procedure:</summary>
<ol>
<li>Click "I want to use a different authenticator app" in the middle of the setup page.{{ mfa_totp_trouble }}<img src="first_auth_app.png"></li>
<li>
...
<li>
<strong>Phone Number</strong>：You can add your phone number and receive an SMS or call (voice guidance) to verify your identity.
<details>
<summary>Setup Procedure:</summary>
<ol>
...
**Be sure to add more than one verification method**. MFA works with just one verification method, but having only one verification method puts you at risk of getting completely locked out of your UTokyo Account when your verification method does not work (due to malfunction, phone number change, etc.). Adding multiple verification methods reduces this risk.
...
<summary>Setup Procedure:</summary>
<ol>
<li>Install the “Microsoft Authenticator” app on your smartphone (if you haven't already done so). The Android version can be downloaded from Google Play</a>（<a href="https://play.google.com/store/apps/details?id=com.azure.authenticator">and the iPhone version from App Store</a><a href="https://apps.apple.com/app/microsoft-authenticator/id983156458">App Store.</li>
<li><a href="https://mysignins.microsoft.com/security-info?domain_hint=utac.u-tokyo.ac.jp">Click "Add method" in theSecurity info page</a>．<img src="alt_add_method.png"></li>
...
<summary>Setup Procedure:</summary>
<ol>
<li><a href="https://mysignins.microsoft.com/security-info?domain_hint=utac.u-tokyo.ac.jp">Click "Add method" in the多要素認証の設定ページSecurity info page</a>．<img src="alt_add_method.png"></li>
<li>For "Which method would you like to add?", select "Authenticator app", and click "Add".<img src="alt_add_method_pull_down.png"></li>
<li>Click "I want to use a different authenticator app".{{ mfa_totp_trouble }}<img src="alt_auth_app.png"></li>
<li>
    Follow the instructions until you reach the QR code page. Scan the QR code with your authenticator app and complete the setup procedure.<br>
    ※「YubiKey」と「Yubico Authenticator」アプリを使う場合の設定手順については，「<a href="yubikey-totp">UTokyo Account多要素認証におけるYubico Authenticatorアプリの利用方法</a>」をご覧ください．
...
<li>
<strong>Phone Number</strong>：You can add your phone number and receive an SMS or call (voice guidance) to verify your identity. There are three types of phones that can be added- “Phone”, “Alternate phone”, and “Office phone”. You may add one phone number for each type (i.e. a total of three phone numbers can be added as your verification method). The description of each phone type is as follows:
<ul>
...
1. If you are taken to the “Security info” page, you have successfully signed in.
{:.mfa-initial-procedure}

<div class="box">
The setup process is not over. Continue to Step 4.
<br><br>
※ If you could not complete this Sep 3 properly, do not proceed to Step 4.Contact theTechnical Support Desk<a href="/support/">．
</div>
...
If the remote-access environment (Citrix Workspace) of office work devices for administrative staff is online, it will be disconnected once you apply for the MFA. Before moving on to the following procedures, please make sure you are signed out.
